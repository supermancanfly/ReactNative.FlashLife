...
function IMChatHomeComponent(props) {
  ...
  const [temFriends, setFriends] = useState([]);
  const emptyStateConfig = {
    title: IMLocalized('No Conversations'),
    description: IMLocalized(
      'Add some users and start chatting with them. Your conversations will show up here.',
    ),
    buttonName: IMLocalized('Add users'),
      ...
      <ScrollView style={styles.container}>
        <View style={styles.searchBarContainer}>
          <SearchBarAlternate
            ...
            placeholderTitle={IMLocalized('Search for users')}
            appStyles={appStyles}
          />
        </View>
