...
export default class FriendshipTracker {
  ...
  addFriendRequest = (fromUser, toUser, callback) => {
    if ((fromUser.userID || fromUser.id) == toUser.userID) {
      callback(null);
      return;
    }
    const state = this.reduxStore.getState();
    const friendships = state.friends.friendships;
    const detectedFriendship = friendships && friendships.find(
      (friendship) => friendship.user.userID == toUser.userID,
    );
    console.log("detected Frinds", JSON.stringify(detectedFriendship));
    if (
      detectedFriendship &&
