...
const createOrUpdateUser = (userFData) => {
  return new Promise(function (resolve, _reject) {
    firebaseAuth.createOrUpdateUser(userFData).then((response) => {
      if (!response.error) {
        handleSuccessfulLogin({ ...response.user }, response.accountCreated).then((res) => {
          // Login successful, push token stored, login credential persisted, so we log the user in.
          resolve({ user: res.user });
        });
      } else {
