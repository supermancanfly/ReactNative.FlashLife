...
export default class TagFormsView extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            employeenumber: "",
    ...
    async onBackFormQuestion() {
        await this.setState({ isFormsQuestionView: false, isTagFormsView: true })
    }
    getClubFormSubmissions() {
        const { userData, employeeData, formDetails } = this.state;
        this.setState({
        ...
        ApiGetFormsList(userData[0].token, params).then(response => {
            ...
            if (response.length > 0) {
                for (let i = 0; i < response.length; i++) {
                    if (response[i].club == 1 && response[i].form_tag == formDetails.form_tag) {
                ...
                renderItem={({ item, index }) => {
                    if (item.assigned_user_types != "" && item.active == 1) {
                        return (
                            ...
                            >
                                <Text style={{ fontSize: 16, color: '#616161' }}>{item.form_tag}</Text>
                                <View style={{
                                    alignItems: 'center',
                                    flexDirection: 'row',
